...
<li x-data="{ {{ $subNavLabel }} : {{ (request()->is($activeRule)) ? 'true' : 'false' }} }">
    <div class="inline-flex w-full">
        <a
                ...
                @endif
                {{ $attributes->merge(['class' => 'px-4 py-3 flex flex-grow items-center space-x-3 rtl:space-x-reverse rounded transition-color duration-200 hover:text-white cursor-default ' . (request()->is($activeRule) ? 'text-white' : 'text-current')]) }}
        >
